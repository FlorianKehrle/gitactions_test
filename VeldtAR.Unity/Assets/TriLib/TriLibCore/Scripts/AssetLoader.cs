...
namespace TriLibCore
{
    ...
    public static class AssetLoader
    {
...
#if UNITY_EDITOR
        private static void SaveScriptableObjectAsset(ScriptableObject scriptableObject, string subFolder)
        {
            string readersFilePath;
            var triLibReadersAssets = AssetDatabase.FindAssets("TriLibReaders");
            if (triLibReadersAssets.Length > 0)
            {
                readersFilePath = AssetDatabase.GUIDToAssetPath(triLibReadersAssets[0]);
            }
            else
            {
                throw new Exception("Could not find TriLibReaders.cs file. Please re-import TriLib package.");
            }
            var readersDirectory = FileUtils.GetFileDirectory(readersFilePath);
            var resourcesDirectory = $"{readersDirectory}/../Resources";
            if (!AssetDatabase.IsValidFolder(resourcesDirectory))
            {
                AssetDatabase.CreateFolder("", resourcesDirectory);
            }
            var mappersDirectory = $"{resourcesDirectory}/Mappers";
            if (!AssetDatabase.IsValidFolder(mappersDirectory))
            {
                AssetDatabase.CreateFolder("", mappersDirectory);
            }
            var assetDirectory = $"{mappersDirectory}/{subFolder}";
            if (!AssetDatabase.IsValidFolder(assetDirectory))
            {
                AssetDatabase.CreateFolder("", assetDirectory);
            }
            var assetPath = $"{assetDirectory}/{scriptableObject.name}.asset";
        ...
        public static AssetLoaderOptions CreateDefaultLoaderOptions(bool generateAssets = false)
        {
            ...
            assetLoaderOptions.FixedAllocations = new List<Object>();
            assetLoaderOptions.RootBoneMapper = ScriptableObject.CreateInstance<ByNameRootBoneMapper>();
            assetLoaderOptions.RootBoneMapper.name = "ByNameRootBoneMapper";
#if UNITY_EDITOR
            if (generateAssets)
            {
                SaveScriptableObjectAsset(assetLoaderOptions.RootBoneMapper, "RootBone");
            }
#endif
            if (MaterialMapper.RegisteredMappers.Count == 0)
            {
                Debug.LogWarning("Please add a callback to the MaterialMapper.RegisteredMappers static field to create the right MaterialMapper for the Render Pipeline you are using.");
            }
            else
            {
                ...
                foreach (var materialMapperName in MaterialMapper.RegisteredMappers)
                {
                    ...
                    var materialMapper = ScriptableObject.CreateInstance(materialMapperName) as MaterialMapper;
                    if (materialMapper != null)
                    {
                        if (materialMapper.IsCompatible(null))
                        {
                            ...
                            if (generateAssets)
                            {
                                SaveScriptableObjectAsset(materialMapper, "Material");
                            }
#endif
            ...
            if (generateAssets)
            {
                SaveScriptableObjectAsset(legacyToHumanoidAnimationClipMapper, "AnimationClip");
                SaveScriptableObjectAsset(simpleAnimationPlayerAnimationClipMapper, "AnimationClip");
            }
#endif
            ...
            assetLoaderOptions.FixNormalMaps = GraphicsSettingsUtils.IsUsingStandardPipeline;
        ...
        private static void ParseGeometry(AssetLoaderContext assetLoaderContext, GameObject meshGameObject, IModel meshModel)
        {
            ...
            if (geometries != null)
            {
                ...
                geometries = mesh.CombineGeometries(geometryGroup, meshModel.BindPoses, assetLoaderContext.Options);
        ...
        private static void ProcessMaterials(AssetLoaderContext assetLoaderContext)
        {
            if (assetLoaderContext.Options.MaterialMappers != null)
            {
                foreach (var material in assetLoaderContext.RootModel.AllMaterials)
                {
                    ...
                    {
                        if (!materialMapper.IsCompatible(materialMapperContext))
                        {
                            continue;
                        }
